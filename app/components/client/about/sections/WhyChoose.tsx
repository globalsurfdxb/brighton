    ...
    <section className="w-full py-100 bg-cream-background overflow-hidden">
      <div className="container">
        ...
        <Swiper
          ...
          className="!pr-px !overflow-visible md:!overflow-hidden"
        >
          {whyChooseData.items.map((item, i) => (
            <SwiperSlide key={i}>
              <div className="border border-secondary rounded-[10px] p-40 h-[280px] sm:h-[300px] lg:h-[340px] 2xl:h-[440px] 3xl:h-[473px] -mr-px relative">
                <div className="absolute inset-0 rounded-[10px]" style={{background: "linear-gradient(180deg, #FFFFFF 0%, #F5F5F5 100%)"}} />
                <div className="relative h-full flex flex-col justify-between">
                    <div className="3xl:w-17.5 3xl:h-17.5 2xl:w-15 2xl:h-15 w-12 h-12 rounded-[5px] bg-primary flex items-center justify-center">
                      <Image
                        src={item.icon}
                        alt={item.title}
                        width={48}
