...
export default function ProjectDetail() {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<NewsForm>({ defaultValues });
  const [isSaving, setIsSaving] = useState(false);
  const [topics, setTopics] = useState<Option[]>([]);

  const fetchOptions = async () => {
    try {
      const [topicsRes] = await Promise.all([fetch("/api/admin/news/topics")]);
  ...
  const fetchNews = async () => {
    try {
      const res = await fetch(`/api/admin/news/news-item/${params.id}`);
  ...
  const onSubmit = async (formData: NewsForm) => {
    setIsSaving(true);
    try {
      const res = await fetch(
        isNew
          ? "/api/admin/news/news-item"
          : `/api/admin/news/news-item/${params.id}`,
        {
      ...
      if (res.ok) {
        toast.success(message || "News saved");
        if (isNew && data?._id) {
          router.replace(`/4dm1n-br1ght0n/news/${data._id}`);
        }
      } else {
        toast.error(message || "Failed to save news");
      }
    } catch (e) {
      console.error(e);
