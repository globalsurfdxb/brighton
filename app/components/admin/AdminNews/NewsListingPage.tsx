...
export default function NewsListingPage() {
  ...
  // ---- News ----
  const toggleHidden = async (e: React.MouseEvent, news: NewsListItem) => {
    e.stopPropagation();
    try {
      const res = await fetch(`/api/admin/news/news-item/${news._id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ isHidden: !news.isHidden }),
      });
      const { message, data } = await res.json();
      if (!res.ok) return toast.error(message || "Failed to update news");
      setNews((prev) =>
        prev.map((p) => (p._id === data._id ? { ...p, ...data } : p)),
      );
      toast.success(
        data.isHidden ? "News hidden successfully" : "News shown successfully",
      );
    } catch (e) {
      console.error(e);
      toast.error("Something went wrong");
  ...
  // ---- Unified delete (topic / news) ----
  const confirmDelete = async () => {
    if (!deleteTarget) return;
    const { type, id } = deleteTarget;
    try {
      if (type === "topic") {
        const res = await fetch(`/api/admin/news/topics?id=${id}`, {
          method: "DELETE",
        });
        const { message, data } = await res.json();
        if (!res.ok) {
        ...
        const res = await fetch(`/api/admin/news/news-item/${id}`, {
          method: "DELETE",
        });
        const { message, data } = await res.json();
        if (!res.ok) {
          toast.error(message || "Failed to delete news");
          return;
        }
        setNews(data);
        ...
        <div className="bg-white border border-secondary rounded-[10px] p-5 flex flex-col gap-4">
          <div className="flex items-center justify-between border-b border-secondary pb-3">
            ...
            <CustomButton
              ...
              text="Add News"
              showIcon={false}
              onClick={() => router.push("/4dm1n-br1ght0n/news/new")}
            />
          </div>
          <div className="grid grid-cols-1 gap-5">
            ...
            {news?.map((news) => (
              <div
                ...
                onClick={() => router.push(`/4dm1n-br1ght0n/news/${news._id}`)}
              >
              ...
              transition-opacity duration-400"
                  ...
                  <button
                    onClick={(e) => toggleHidden(e, news)}
                    type="button"
                    className="cursor-pointer hover:scale-110 transition-all"
                    title={news.isHidden ? "Show news" : "Hide news"}
                  >
                    {news.isHidden ? (
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      ...
                      setDeleteTarget({
                        type: "news",
                        id: news._id,
                        label: news.title || "Untitled News",
                      });
                    }}
                    title="Delete news"
                  >
